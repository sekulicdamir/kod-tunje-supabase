...
import { MenuIcon, CloseIcon, PhoneIcon, InstagramIcon, FacebookIcon, MapPinIcon, ChevronDownIcon, EmailIcon, ChevronLeftIcon, ChevronRightIcon, ViberIcon, WhatsAppIcon, TelegramIcon, MessengerIcon } from './components/Icons';

// --- SUPABASE CONFIGURATION ---
const supabaseUrl = 'https://danroplspvdqreitsbmv.supabase.co';
const supabaseKey = 'sb_publishable_YGbMLkLB0F86MAEhTqXNJw_ZdGgQn26'; // Note: In production, ensure RLS policies are set correctly.
...
const DEFAULT_SITE_DATA: SiteData = {
  heroImage: "https://ik.imagekit.io/j6wdxvinv/Kod%20Tunje/Konoba%20kod%20Tunje%20Hero%20Homepage.jpg",
  ...
  viberLink: "viber://chat?number=%2B38267123456",
  whatsappLink: "https://wa.me/38267123456",
  telegramLink: "https://t.me/username",
  messengerLink: "https://m.me/username",
};

// Custom hook for Supabase
function useSupabaseData(initialValue: SiteData) {
  const [data, setData] = useState<SiteData>(initialValue);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        // We assume a table named 'site_content' exists with columns 'id' and 'data' (jsonb).
        ...
        if (fetchError) {
          // Check for 'PGRST116' (No rows) or table missing error (42P01) or text message
          if (fetchError.code === 'PGRST116' || fetchError.message.includes('Could not find the table') || fetchError.code === '42P01') {
             console.log("No data found or table missing, using default.");
             // We stick with initialValue
          } else {
             throw fetchError;
          }
        } else if (rows) {
          setData(rows.data as SiteData);
        }
      } catch (err: any) {
  ...
  const updateData = async (newData: SiteData) => {
    try {
      ...
      const { error: updateError } = await supabase
        .from('site_content')
        .upsert({ id: 1, data: newData }); // upsert ensures it creates if not exists

      if (updateError) {
         if (updateError.message.includes('Could not find the table') || updateError.code === '42P01') {
            console.warn("Table 'site_content' missing. Data not saved to backend.");
            alert("Warning: The database table 'site_content' does not exist in Supabase. Your changes are saved locally for this session but won't persist to the server. Please create the table in Supabase.");
         } else {
            throw updateError;
         }
      } else {
  ...
  return { siteData: data, updateSiteData: updateData, loading, error };
}
...
// Helper component for Image Input with Supabase Upload
const ImageInput = ({ value, onChange, label, className }: { value: string, onChange: (val: string) => void, label?: string, className?: string }) => {
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            console.error(err);
            alert('Upload failed. Please ensure an "images" bucket exists in your Supabase project with public access.');
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <div className={className}>
            {label && <label className="block text-lg font-medium text-gray-600 mb-1">{label}</label>}
            <div className="flex gap-2">
                <input 
                    value={value} 
                    onChange={(e) => onChange(e.target.value)} 
                    className="flex-grow p-3 border border-gray-300 rounded-md shadow-sm text-lg"
                    placeholder="Image URL"
                />
                <label className={`flex-shrink-0 bg-blue-100 hover:bg-blue-200 text-blue-800 font-bold py-3 px-4 rounded-md cursor-pointer flex items-center justify-center min-w-[100px] ${uploading ? 'opacity-50' : ''}`}>
...
const AdminDashboard = ({ data, onSave }: {data: SiteData, onSave: (data: SiteData) => Promise<void>}) => {
    ...
    return (<div className="bg-gray-100 min-h-screen p-4"><div className="max-w-6xl mx-auto bg-white p-4 sm:p-6 rounded-lg shadow-xl"><div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 border-b pb-4"><h1 className="text-2xl sm:text-3xl font-bold text-gray-800 text-center sm:text-left">Admin Dashboard</h1><div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto"><a href="/#" className="text-blue-600 hover:underline w-full sm:w-auto text-center py-2 text-lg">← Back to Site</a><button onClick={handleSave} disabled={isSaving} className={`${isSaving ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold py-3 px-6 rounded-lg w-full sm:w-auto text-xl`}>{isSaving ? 'Saving...' : 'Save'}</button></div></div>{message && <div className="bg-green-100 border-green-400 text-green-700 px-4 py-3 rounded mb-4 text-center">{message}</div>}<div className="space-y-10">{/* General Settings */}<div className="p-4 border rounded-lg"><h2 className="text-xl sm:text-2xl font-semibold mb-4 text-gray-700">General Settings</h2><div className="grid md:grid-cols-2 gap-6"><div><ImageInput label="Logo Image URL" value={formData.logoUrl} onChange={val => handleFieldChange('logoUrl', val)} /></div><div><ImageInput label="Hero Image URL" value={formData.heroImage} onChange={val => handleFieldChange('heroImage', val)} /></div></div></div>
    
    {/* Social Links Settings */}
    <div className="p-4 border rounded-lg"><h2 className="text-xl sm:text-2xl font-semibold mb-4 text-gray-700">Social Links</h2><div className="grid md:grid-cols-2 gap-6"><div><label className="block text-lg font-medium text-gray-600 mb-1">Viber Link</label><input value={formData.viberLink} onChange={e => handleFieldChange('viberLink', e.target.value)} className="w-full p-3 border border-gray-300 rounded-md shadow-sm text-lg"/></div><div><label className="block text-lg font-medium text-gray-600 mb-1">WhatsApp Link</label><input value={formData.whatsappLink} onChange={e => handleFieldChange('whatsappLink', e.target.value)} className="w-full p-3 border border-gray-300 rounded-md shadow-sm text-lg"/></div><div><label className="block text-lg font-medium text-gray-600 mb-1">Telegram Link</label><input value={formData.telegramLink} onChange={e => handleFieldChange('telegramLink', e.target.value)} className="w-full p-3 border border-gray-300 rounded-md shadow-sm text-lg"/></div><div><label className="block text-lg font-medium text-gray-600 mb-1">Messenger Link</label><input value={formData.messengerLink} onChange={e => handleFieldChange('messengerLink', e.target.value)} className="w-full p-3 border border-gray-300 rounded-md shadow-sm text-lg"/></div></div></div>
    
    {/* Page Content */}<div className="p-4 border rounded-lg"><h2 className="text-xl sm:text-2xl font-semibold mb-4 text-gray-700">Page Content (Montenegrin)</h2><div className="grid md:grid-cols-2 gap-6">{Object.keys(formData.translations.me).map(key => (<div key={key}><label className="block text-lg font-medium text-gray-600 capitalize mb-1">{key.replace(/([A-Z])/g, ' $1')}</label><textarea value={formData.translations.me[key]} onChange={e => handleTextChange('me', key, e.target.value)} className="w-full p-3 border border-gray-300 rounded-md shadow-sm text-lg h-24"/></div>))}</div></div>
    
    {/* Image Lists */}
    <div className="grid md:grid-cols-2 gap-8">
        <div className="p-4 border rounded-lg space-y-4">
            <h2 className="text-xl sm:text-2xl font-semibold text-gray-700">Slider Images</h2>
            {Array.isArray(formData.sliderImages) && formData.sliderImages.map((url, i) => (
                <div key={i} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <ImageInput value={url} onChange={val => handleListChange('sliderImages', i, val)} className="flex-grow"/>
                    <button onClick={() => removeListItem('sliderImages', i)} className="bg-red-500 text-white p-3 rounded-md w-full sm:w-auto h-[54px] self-end sm:self-auto">X</button>
                </div>
            ))}
            <button onClick={() => addListItem('sliderImages')} className="mt-2 bg-green-500 text-white px-5 py-3 rounded-lg text-lg w-full sm:w-auto">+ Add</button>
        </div>
        <div className="p-4 border rounded-lg space-y-4">
            <h2 className="text-xl sm:text-2xl font-semibold text-gray-700">Gallery Images</h2>
            {Array.isArray(formData.galleryImages) && formData.galleryImages.map((url, i) => (
                <div key={i} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <ImageInput value={url} onChange={val => handleListChange('galleryImages', i, val)} className="flex-grow"/>
                    <button onClick={() => removeListItem('galleryImages', i)} className="bg-red-500 text-white p-3 rounded-md w-full sm:w-auto h-[54px] self-end sm:self-auto">X</button>
                </div>
            ))}
            <button onClick={() => addListItem('galleryImages')} className="mt-2 bg-green-500 text-white px-5 py-3 rounded-lg text-lg w-full sm:w-auto">+ Add</button>
        </div>
    </div>
    
    <div className="p-4 border rounded-lg space-y-4">
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-700">Slider 2 Images</h2>
        {Array.isArray(formData.slider2Images) && formData.slider2Images.map((url, i) => (
            <div key={i} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                <ImageInput value={url} onChange={val => handleListChange('slider2Images', i, val)} className="flex-grow"/>
                <button onClick={() => removeListItem('slider2Images', i)} className="bg-red-500 text-white p-3 rounded-md w-full sm:w-auto h-[54px] self-end sm:self-auto">X</button>
            </div>
        ))}
        <button onClick={() => addListItem('slider2Images')} className="mt-2 bg-green-500 text-white px-5 py-3 rounded-lg text-lg w-full sm:w-auto">+ Add</button>
    </div>
    
    <div className="p-4 border rounded-lg space-y-4">
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-700">Menu Block Images</h2>
        {Array.isArray(formData.menuImages) && formData.menuImages.map((url, i) => (
            <div key={i} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                 <ImageInput value={url} onChange={val => handleListChange('menuImages', i, val)} className="flex-grow"/>
                <button onClick={() => removeListItem('menuImages', i)} className="bg-red-500 text-white p-3 rounded-md w-full sm:w-auto h-[54px] self-end sm:self-auto">X</button>
            </div>
        ))}
        <button onClick={() => addListItem('menuImages')} className="mt-2 bg-green-500 text-white px-5 py-3 rounded-lg text-lg w-full sm:w-auto">+ Add</button>
    </div>
    
    {/* Opening Hours */}<div className="p-4 border rounded-lg"><h2 className="text-xl sm:text-2xl font-semibold mb-4 text-gray-700">Opening Hours</h2><div className="space-y-4">{formData.openingHours.map((item, i) => (<div key={i} className="flex flex-col sm:flex-row gap-2"><input value={item.day} onChange={e => handleOpeningHoursChange(i, 'day', e.target.value)} className="w-full sm:w-1/3 p-3 border rounded-md text-lg" placeholder="Day"/><input value={item.hours} onChange={e => handleOpeningHoursChange(i, 'hours', e.target.value)} className="w-full sm:w-2/3 p-3 border rounded-md text-lg" placeholder="Hours"/><button onClick={() => removeOpeningHourItem(i)} className="bg-red-500 text-white p-3 rounded-md w-full sm:w-auto">X</button></div>))}<button onClick={addOpeningHourItem} className="mt-4 bg-green-500 text-white px-5 py-3 rounded-lg text-lg w-full sm:w-auto">+ Add</button></div></div>{/* Menu */}<div className="p-4 border rounded-lg"><h2 className="text-xl sm:text-2xl font-semibold mb-4 text-gray-700">Menu</h2><button onClick={() => addListItem('menu')} className="mb-4 bg-green-500 text-white px-5 py-3 rounded-lg text-lg w-full sm:w-auto">+ Add Menu Item</button><div className="grid lg:grid-cols-2 gap-x-8 gap-y-6">{Object.entries(formData.menu).map(([cat, items]) => (<div key={cat}><h3 className="font-bold capitalize text-xl mb-3">{cat}</h3><div className="space-y-4">{Array.isArray(items) && items.map((item, i) => (<div key={i} className="flex flex-col sm:flex-row gap-2"><input value={item.name} onChange={e => handleListChange('menu', i, {...item, name: e.target.value}, cat as keyof Menu)} className="w-full sm:w-2/3 p-2 border rounded-md text-lg"/><input value={item.price} onChange={e => handleListChange('menu', i, {...item, price: e.target.value}, cat as keyof Menu)} className="w-full sm:w-1/3 p-2 border rounded-md text-lg"/><button onClick={() => removeListItem('menu', i, cat as keyof Menu)} className="bg-red-500 text-white p-2 rounded-md w-full sm:w-auto">X</button></div>))}</div></div>))}</div></div></div></div></div>);
};
...
const Lightbox = ({ images, initialIndex, onClose }: { images: string[], initialIndex: number, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-95 flex items-center justify-center touch-none" onClick={onClose}>
        <button className="absolute top-4 right-4 text-white p-2 hover:bg-white/10 rounded-full z-50" onClick={onClose}>
            <CloseIcon className="w-10 h-10" />
        </button>
        
        <button className="absolute left-2 md:left-8 text-white p-2 md:p-4 hover:bg-white/10 rounded-full transition-colors z-50" onClick={handlePrev}>
            <ChevronLeftIcon className="w-10 h-10 md:w-16 md:h-16" />
        </button>
        ...
        <button className="absolute right-2 md:right-8 text-white p-2 md:p-4 hover:bg-white/10 rounded-full transition-colors z-50" onClick={handleNext}>
            <ChevronRightIcon className="w-10 h-10 md:w-16 md:h-16" />
        </button>
    </div>
  );
};
...
const HomePage = () => {
    ...
    return (<main><section className="h-screen bg-cover bg-center flex items-center justify-center text-white" style={{ backgroundImage: `url('${data.heroImage}')` }}><div className="bg-black bg-opacity-60 text-center p-8 rounded-lg max-w-4xl mx-auto"><h1 className="text-4xl md:text-7xl font-bold mb-4">{t('heroTitle')}</h1><a href={t('heroCtaLink')} className="bg-[#075e54] hover:bg-[#054c43] text-white font-bold py-3 px-8 rounded-md inline-flex items-center justify-center mt-4"><PhoneIcon className="w-5 h-5 mr-2" /><span>{t('heroCtaText')}</span></a></div></section><section className="py-16 md:py-24 bg-[#6B1F1F] text-white"><div className="container mx-auto px-4 text-center max-w-4xl"><h2 className="text-3xl md:text-4xl font-bold mb-6">{t('aboutTitle')}</h2><p className="mb-4 text-lg text-gray-300">{t('aboutText1')}</p><p className="text-lg text-gray-300">{t('aboutText2')}</p></div></section>
      ...
      <section className="py-16 bg-[#F8F5F2]"><div className="container mx-auto px-4 text-center"><a href={t('reservationsCtaLink')} className="bg-[#075e54] hover:bg-[#054c43] text-white font-bold py-4 px-10 text-2xl rounded-lg inline-flex items-center justify-center transition-transform hover:scale-105"><PhoneIcon className="w-6 h-6 mr-3" /><span>{t('reservationsCtaText')}</span></a></div></section><section className="bg-black text-white py-20 md:py-28"><div className="container mx-auto px-4 text-center max-w-3xl"><p className="text-3xl md:text-4xl font-bold">{t('quoteText')}</p></div></section>
      <section className="container mx-auto px-4 py-16 md:py-24">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {Array.isArray(data.galleryImages) && data.galleryImages.map((src, i) => (
            <div key={i} className="aspect-square overflow-hidden rounded-lg shadow-lg cursor-pointer" onClick={() => setLightboxIndex(i)}>
              <img src={src} alt={`G image ${i+1}`} className="w-full h-full object-cover hover:scale-105 transition-transform"/>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-[#F8F5F2]"><div className="container mx-auto px-4"><div className="flex flex-col md:flex-row justify-center items-center gap-8 md:gap-12 max-w-5xl mx-auto"><div className="w-full max-w-md"><OpeningHoursCard hours={data.openingHours} /></div><div className="w-full max-w-md grid grid-cols-2 gap-4"><SocialButton icon={<ViberIcon className="w-full h-full" />} label="Viber" link={data.viberLink} color="bg-[#7360f2]" /><SocialButton icon={<WhatsAppIcon className="w-full h-full" />} label="WhatsApp" link={data.whatsappLink} color="bg-[#25D366]" /><SocialButton icon={<TelegramIcon className="w-full h-full" />} label="Telegram" link={data.telegramLink} color="bg-[#0088cc]" /><SocialButton icon={<MessengerIcon className="w-full h-full" />} label="Facebook Messenger" link={data.messengerLink} color="bg-[#006AFF]" /></div></div></div></section>
...
const SocialButton = ({ icon, label, link, color }: { icon: React.ReactNode, label: string, link: string, color: string }) => (
    <a href={link} target="_blank" rel="noopener noreferrer" className={`${color} text-white p-4 rounded-lg shadow-md flex flex-col items-center justify-center hover:opacity-90 transition-opacity gap-2`}>
...
const ContactPage = () => {
    ...
    return (<main className="py-24 pt-8 bg-white" style={{minHeight: 'calc(100vh - 80px)'}}><div className="container mx-auto px-4 max-w-4xl"><h1 className="text-4xl md:text-6xl font-bold mb-12 text-center">{t('contactPageTitle')}</h1><div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12"><SocialButton icon={<ViberIcon className="w-full h-full" />} label="Viber" link={data.viberLink} color="bg-[#7360f2]" /><SocialButton icon={<WhatsAppIcon className="w-full h-full" />} label="WhatsApp" link={data.whatsappLink} color="bg-[#25D366]" /><SocialButton icon={<TelegramIcon className="w-full h-full" />} label="Telegram" link={data.telegramLink} color="bg-[#0088cc]" /><SocialButton icon={<MessengerIcon className="w-full h-full" />} label="Facebook Messenger" link={data.messengerLink} color="bg-[#006AFF]" /></div><div className="bg-[#F8F5F2] p-8 rounded-lg shadow-md mb-12"><div className="space-y-6 text-xl"><p className="flex items-center gap-4"><MapPinIcon className="w-6 h-6 text-[#075e54]" /><span>{t('footerAddress')}</span></p><p className="flex items-center gap-4"><PhoneIcon className="w-6 h-6 text-[#075e54]" /><a href={`tel:${t('phoneValue')}`} className="hover:underline">{t('phoneValue')}</a></p><p className="flex items-center gap-4"><EmailIcon className="w-6 h-6 text-[#075e54]" /><a href={t('footerOrderLink')} className="hover:underline">reservations@kodtunje.me</a></p></div><div className="mt-8"><h3 className="text-2xl font-bold mb-4">{t('companyNameLabel')}</h3><p>{t('companyNameValue')}</p><p>{t('vatNumberLabel')}: {t('vatNumberValue')}</p></div><a href={t('directionsButtonLink')} target="_blank" rel="noopener noreferrer" className="bg-[#075e54] hover:bg-[#054c43] text-white font-bold py-3 px-6 rounded-md inline-flex items-center justify-center mt-8 transition-transform hover:scale-105 text-lg w-full md:w-auto"><MapPinIcon className="w-5 h-5 mr-2" /><span>{t('directionsButtonText')}</span></a></div></div></main>);
};
...
const Header = ({ navigateToHome, navigateToAbout, navigateToMenu, navigateToContact, isHome }: {navigateToHome: () => void, navigateToAbout: () => void, navigateToMenu: () => void, navigateToContact: () => void, isHome: boolean}) => {
    ...
    const navItems = [{ page: 'home', label: t('navHome'), action: navigateToHome }, { page: 'menu', label: t('navMenu'), action: navigateToMenu }, { page: 'about', label: t('navAbout'), action: navigateToAbout }, { page: 'contact', label: t('navContact'), action: navigateToContact }];
    ...
    return (
        <LanguageProvider translations={data.translations}>
            <DataContext.Provider value={data}>
                ...
                <Header navigateToHome={navigateToHome} navigateToAbout={navigateToAbout} navigateToMenu={navigateToMenu} navigateToContact={navigateToContact} isHome={page === 'home'} />
                {page === 'home' && <HomePage />}
                {page === 'menu' && <MenuPage />}
...
const App = () => {
    ...
    const { siteData, updateSiteData, loading, error } = useSupabaseData(DEFAULT_SITE_DATA);
    ...
    if (loading && !siteData) {
      return (
        <div className="flex items-center justify-center min-h-screen bg-[#F8F5F2]">
          <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-[#075e54]"></div>
        </div>
      );
    }
    
    // We render the app even if loading is true (e.g. initial fetch) but use default data if needed or show spinner above.
    // If error, we might want to show it, but for now fallback to default data is safer for UX, or show error in console.
    
    if (route === '#/admin') {
        if (isAuthenticated) return <AdminDashboard data={siteData} onSave={updateSiteData} />;
